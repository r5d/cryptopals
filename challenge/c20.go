...
func C20() {
	...
	// Find the shortest cipher. This cipher's length will
	// become the keys size for the repeating XOR.
	keySize := 2000
	...
	// Make a cipher stream.
	cipherStream := make([]byte, 0)
	for _, cipher := range ciphers {
		cipherStream = append(cipherStream, cipher[0:keySize]...)
	}
	// Crack and barf text stream.
	_, textStream := crack(cipherStream, keySize)
	for i, b := range textStream {
		if i > 0 && i%keySize == 0 {
			fmt.Printf("\n")
		}
