...
func C38(args []string) {
	...
	serverLoginUser := func(server *lib.SRPServer, info []string,
		...
		serverPub, err := user.EphemeralKeyPubSimple()
		if err != nil {
			return fmt.Errorf("server pub key: %v", err)
		}

		// Generate scrambling param.
		err = user.SetScramblingParamSimple()
		if err != nil {
			return fmt.Errorf("setting scrambling param: %v", err)
		}
		sParam := user.GetScramblingParam()

		// Make ACK packet
		packet := fmt.Sprintf("%s+%s+%s", lib.BytesToHexStr(user.Salt()),
			lib.BytesToHexStr(serverPub.Bytes()),
			lib.BytesToHexStr(sParam))
	...
	clientLoginUser := func(client *lib.SRPClient, ident string) error {
		...
		sParam := lib.HexStrToBytes(parts[2])

		// Compute session key.
		err = client.Session.SetScramblingParamSimple(sParam)
		if err != nil {
			return fmt.Errorf("setting scrambling param: %v", err)
		}
		err = client.Session.ComputeSessionKeySimple(salt, pass, serverPub)
		if err != nil {
			return fmt.Errorf("computing session key: %v", err)
		}
